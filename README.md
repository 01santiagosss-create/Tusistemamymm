...
<head>
  ...
  <title>Inventario y Asignación de Equipos (1 archivo)</title>
  ...
  <style>
    ...
    *{box-sizing:border-box}
    ...
    .preview{max-width:220px;border-radius:12px;border:1px solid var(--line);display:block;margin-top:8px}
  ...
  <div class="topbar">
    <div class="brand">
      ...
      <div>
        ...
        <div class="mini">1 archivo (LocalStorage) · QR · Adjuntos · PDF</div>
      </div>
    </div>
    ...
    <div class="grid">
      <div class="card">
        <div class="row">
          <div>
            ...
            <div class="mini">Busca por Asset ID o Serial</div>
          </div>
        </div>
        <div class="sep"></div>
        <div class="row">
        ...
        <div class="mini" style="margin-top:8px">Atajo: <span class="kbd">Enter</span> en el buscador</div>
      </div>
      ...
      <div class="card" id="adminOnlyCard">
        ...
        <div id="tab-assign" class="hidden" style="margin-top:10px">
          ...
          <div class="mini" style="margin-top:10px">
            Tip: “Asignar + PDF” genera el recibo inmediatamente.
          </div>
        </div>
      ...
      <div class="card">
        ...
        <div class="mini muted" style="margin-top:10px">
          Esto te sirve como “mini base de datos” portátil.
        </div>
      </div>
    </div>
      ...
      <div class="card" id="detailCard">
        <div class="row">
          ...
          <div class="right row" style="gap:8px">
            ...
            <button class="btn" id="reportBtn">Reporte PDF</button>
          </div>
        </div>
        <div class="sep"></div>
...
function loadDB(){
  ...
  const db = {
    ...
    assignments: [
      // {id, asset_tag, person, doc, location, notes, assigned_at, returned_at, created_by}
    ],
    attachments: [
      // {id, asset_tag, type, desc, filename, mime, dataUrl, created_at, created_by}
    ],
    session: { userId: null }
  };
  saveDB(db);
...
function openDetail(tag){
  ...
  // link for QR (this page with query param)
...
/* =========================
   5) PDF: Recibo + Reporte + Etiqueta
========================= */
function getQRDataURLFromCanvas(){
  const canvas = document.getElementById("qrCanvas");
...
function pdfReciboAsignacion(asset_tag){
  ...
  const qrData = getQRDataURLFromCanvas();

  const margin = 48;
  let y = 56;

  doc.setFont("helvetica","bold"); doc.setFontSize(16);
  doc.text("RECIBO DE ENTREGA / ASIGNACIÓN DE EQUIPO", margin, y);
  y += 18;

  doc.setFont("helvetica","normal"); doc.setFontSize(10);
  doc.text(`Fecha: ${fmtDate(asg.assigned_at)}`, margin, y);
  doc.text(`Generado por: ${asg.created_by}`, 360, y);
  y += 18;

  doc.setDrawColor(34,48,84);
  doc.roundedRect(margin, y, 500, 120, 10, 10);
  y += 18;

  doc.setFont("helvetica","bold"); doc.setFontSize(12);
  doc.text("Datos del equipo", margin+14, y); y += 14;
  ...
  lines.forEach(l => { doc.text(l, margin+14, y); y += 12; });
  ...
  doc.setFont("helvetica","bold"); doc.setFontSize(12);
  doc.text("Responsable / Ubicación", margin+14, y); y += 14;

  doc.setFont("helvetica","normal"); doc.setFontSize(10);
  const lines2 = [
    `Persona: ${asg.person}`,
    `Documento: ${asg.doc || "—"}`,
    `Ubicación: ${asg.location}`,
    `Notas: ${asg.notes || "—"}`,
  ];
  lines2.forEach(l => { doc.text(doc.splitTextToSize(l, 470), margin+14, y); y += 12; });

  y += 8;
  doc.setFont("helvetica","normal"); doc.setFontSize(10);
  doc.text("Firma entrega: _______________________", margin, y+40);
  doc.text("Firma recibe: ________________________", margin+270, y+40);

  if(qrData){
    doc.addImage(qrData, "PNG", 420, 62, 120, 120);
    doc.setFontSize(9);
    doc.text("QR - Ficha del equipo", 432, 188);
  }
...
function pdfReporteEquipo(asset_tag){
  ...
  doc.setFont("helvetica","bold"); doc.setFontSize(16);
  doc.text("REPORTE DE EQUIPO", margin, y);
  doc.setFont("helvetica","normal"); doc.setFontSize(10);
  doc.text(`Generado: ${fmtDate(nowISO())}`, margin, y+16);
  y += 30;

  if(qrData){
    doc.addImage(qrData, "PNG", 420, 40, 120, 120);
    doc.setFontSize(9);
    doc.text("QR - Ficha", 456, 166);
  }

  doc.setFont("helvetica","bold"); doc.setFontSize(12);
  doc.text("Ficha técnica", margin, y); y += 14;
  ...
  const info = [
    ...
    `IMEI: ${asset.imei || "—"}`,
    `Estado: ${asset.estado}`,
    `Asignación actual: ${asg ? `${asg.person} (${asg.doc||"—"}) · ${asg.location} · Desde ${fmtDate(asg.assigned_at)}` : "No asignado"}`,
    `Observaciones: ${asset.notas || "—"}`
  ];
  info.forEach(l => {
    const t = doc.splitTextToSize(l, 500);
    doc.text(t, margin, y); y += 12 * t.length;
  });

  y += 10;
  doc.setFont("helvetica","bold"); doc.setFontSize(12);
  doc.text("Historial de asignaciones", margin, y); y += 12;
  doc.setFont("helvetica","normal"); doc.setFontSize(10);

  if(hist.length===0){
    doc.text("Sin historial.", margin, y); y += 12;
  } else {
    for(const x of hist){
      const line = `• ${fmtDate(x.assigned_at)}  →  ${x.person} · ${x.location}${x.returned_at?` (Devuelto ${fmtDate(x.returned_at)})`:" (Activo)"}`;
  ...
  y += 10;
  doc.setFont("helvetica","bold"); doc.setFontSize(12);
  doc.text("Adjuntos", margin, y); y += 12;
  doc.setFont("helvetica","normal"); doc.setFontSize(10);

  if(atts.length===0){
    doc.text("Sin adjuntos.", margin, y); y += 12;
  } else {
    for(const a of atts){
...
function doSearch(q){
  if(!q){ toast("Escribe un Asset ID o Serial"); return; }
